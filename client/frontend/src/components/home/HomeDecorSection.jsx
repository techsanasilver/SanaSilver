...
/**
 * HomeDecorSection Component
 * Displays subcategories of Home Decor in a carousel
 */
const HomeDecorSection = () => {
    const { getCategoryBySlug, loading } = useCategories();
    const [subcategories, setSubcategories] = useState([]);

    // Get Home Decor category and its subcategories
    useEffect(() => {
        if (!loading) {
            const homeDecorCategory = getCategoryBySlug("home-decor");
            if (homeDecorCategory && homeDecorCategory.children) {
                setSubcategories(homeDecorCategory.children);
                logger.info(
                    `Loaded ${homeDecorCategory.children.length} home decor subcategories`,
                );
            } else {
                logger.warn("Home Decor category not found or has no children");
            }
        }
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="text-center mb-8 md:mb-12">
                    ...
                    <p className="text-sm md:text-base text-text-muted font-light max-w-2xl mx-auto">
                        Transform your space with our elegant home decor pieces,
                        blending tradition and contemporary aesthetics
                    </p>
                </div>

                {/* Conditional Rendering: Grid for few items, Carousel for many */}
                {subcategories.length < 4 ? (
                    // Centered Grid for few items
                    <div className="flex justify-center gap-4 md:gap-6 max-w-4xl mx-auto px-4">
