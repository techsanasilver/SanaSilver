...
/**
 * JewellerySection Component
 * Displays subcategories of Jewellery in a carousel
 */
const JewellerySection = () => {
    const { getCategoryBySlug, loading } = useCategories();
    const [subcategories, setSubcategories] = useState([]);

    // Get Jewellery category and its subcategories
    useEffect(() => {
        if (!loading) {
            const jewelleryCategory = getCategoryBySlug("jewellery");
            if (jewelleryCategory && jewelleryCategory.children) {
                setSubcategories(jewelleryCategory.children);
                logger.info(
                    `Loaded ${jewelleryCategory.children.length} jewellery subcategories`,
                );
            } else {
                logger.warn("Jewellery category not found or has no children");
            }
        }
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="text-center mb-8 md:mb-12">
                    ...
                    <p className="text-sm md:text-base text-text-muted font-light max-w-2xl mx-auto">
                        Discover our exquisite range of jewellery, from timeless
